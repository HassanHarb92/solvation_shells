...
def run_xtb_and_move(new_xyz_path, original_xyz_path):
    ...
    namespace_prefix = os.path.splitext(new_xyz_path)[0]
#    subprocess.run(['xtb', new_xyz_path, '--opt', '--gfn2', '--chrg', '2', '--alpb', 'water', '--cycles','1000','--gfniter', '3000'], check=True)
    subprocess.run(['xtb', new_xyz_path, '--opt', '--gfn2', '--chrg', '2',  '--cmaes','--maxstep', '0.2', '200'  , '--cycles', '1000', '--gfniter', '3000', '--namespace', namespace_prefix], check=True)
    # Define new file names and paths
    optimized_file = f'{namespace_prefix}.xtbopt.xyz'
