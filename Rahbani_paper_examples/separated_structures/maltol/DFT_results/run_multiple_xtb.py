...
def create_frozen_input(filename, frozen_count):
    ...
    with open("maltolate_frozen.inp", "w") as f:
        f.write("$fix\n")
        f.write(f"  atoms: 1-{frozen_count}\n")
        f.write("$end\n")

def run_xtb_optimization(input_file, output_file, charge=-1, frozen_input="maltolate_frozen.inp"):
...
def main(input_xyz, script_path):
    ...
    shutil.copy(input_xyz, "maltolate_original.xyz")
    shutil.copy(input_xyz, "maltolate_optimized.xyz")
