...
def create_frozen_input(filename, frozen_count):
    ...
    with open("catecholate_frozen.inp", "w") as f:
        f.write("$fix\n")
        f.write(f"  atoms: 1-{frozen_count}\n")
        f.write("$end\n")

def run_xtb_optimization(input_file, output_file, charge=-4, frozen_input="catecholate_frozen.inp"):
...
def main(input_xyz, script_path):
    ...
    shutil.copy(input_xyz, "catecholate_original.xyz")
    shutil.copy(input_xyz, "catecholate_optimized.xyz")
