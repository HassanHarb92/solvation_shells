...
def create_frozen_input(filename, frozen_count):
    ...
    with open("kojate_frozen.inp", "w") as f:
        f.write("$fix\n")
        f.write(f"  atoms: 1-{frozen_count}\n")
        f.write("$end\n")

def run_xtb_optimization(input_file, output_file, charge=-1, frozen_input="kojate_frozen.inp"):
...
def main(input_xyz, script_path):
    ...
    shutil.copy(input_xyz, "kojate_original.xyz")
    shutil.copy(input_xyz, "kojate_optimized.xyz")
    # Step 4: XTB Optimization on the original structure
    optimized_file_1 = "kojate_optimized.xyz"
#    run_xtb_optimization(input_xyz, optimized_file_1)

    # Step 5: Add the first solvation shell
    solvated_file_1 = "kojate_solvated_1.xyz"
    ...
    optimized_file_4 = "kojate_solvated_3_optimized.xyz"
